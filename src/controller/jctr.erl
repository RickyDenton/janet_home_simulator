...
%%               - CtrSublocTable: The serialized controller's 'ctr_sublocation' table ([{subloc_id,subloc_devlist}])
...
%%
run(Loc_id,CtrSublocTable,MgrPid,RestPort,RemoteHost) when is_number(Loc_id), Loc_id>0, is_pid(MgrPid), is_number(RestPort), RestPort>=30000 ->
 ...
 case utils:is_running(janet_controller) of
  ...
  false ->
   ...
   ok = ctr_db:init_mnesia(CtrSublocTable),
